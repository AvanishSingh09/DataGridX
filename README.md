...
**DataViewX** is a fast, responsive, and privacy-first CSV data analysis and visualization web application built with **React 19** and **Vite**.

Unlike conventional online tools that upload sensitive spreadsheets to external cloud servers, **DataViewX processes 100% of your data client-side inside your browser**. Your files never leave your computer.

Whether you are analyzing sales figures, student records, logs, or sensor metrics, DataViewX provides instant multi-column filtering, numeric sorting, real-time KPI metrics, dynamic chart generation (Bar, Area, Donut), and filtered CSV export.
...
> **Note:** No installation or account needed. Drop any `.csv` file or click **"Load Sample Dataset"** to start exploring immediately.
...
- 🔢 **Numeric-Aware Sorting:** Click headers to cycle through Ascending → Descending → Default. Correctly handles numbers (`20` < `100`).
...
- [x] **Upload Test:** Drop a CSV or click **"Load Sample Dataset"** → verifies column detection and row rendering.
- [x] **Filter Test:** Type `Delhi` in City and `CSE` in Course → table displays only matching rows (AND logic).
- [x] **Case-Insensitive Test:** Type `delhi` or `DELHI` → verifies case-insensitivity.
- [x] **Sorting Test:** Click **Age** header → sorts numerically (`20`, `21`, `22`, `23`).
- [x] **Visual Analytics Test:** Switch to **Visual Analytics** tab → inspect charts, KPI metric cards, and category breakdown.
- [x] **Dynamic Axis Test:** Change Group By to `Course` and Value to `Score` → chart plots average score per course.
- [x] **Theme Switcher Test:** Click the Sun/Moon button → verifies instant dark/light mode toggle with stored preference.
- [x] **Export Test:** Click **"Export Filtered CSV"** → downloads only active filtered rows.
- [x] **Error Handling Test:** Try uploading a `.txt` file → prompts `"Please upload a CSV file."`.
